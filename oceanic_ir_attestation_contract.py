...
def proof_from_attestation(attestation: Mapping[str, Any]) -> ProofArtifact:
    ...
    claims = (
        ProofClaim(
            obligation="attestation_signature",
            status="satisfied" if signature else "missing",
            evidence_type="signature",
            evidence_digest=_digest(signature),
        ),
        ProofClaim(
            obligation="attestation_lineage",
            status="satisfied" if lineage else "missing",
            evidence_type="lineage",
            evidence_digest=_digest(lineage),
        ),
    )
    adapter = AdapterManifest(
        ...
        adapter_version="1",
        capabilities=("attestation_signature", "attestation_lineage"),
        proof_types=("signature", "lineage"),
        limitations=(),
    )
    return ProofArtifact(
        contract_id=CONTRACT_ID,
        adapter=adapter,
        implementation_digest=_digest(attestation),
        toolchain_digest=_digest("omega-edge"),
        claims=claims,
    )
...
def verify_attestation_contract(attestation: Mapping[str, Any]):
    ...
    return validator.verify(contract(), proof_from_attestation(attestation))
