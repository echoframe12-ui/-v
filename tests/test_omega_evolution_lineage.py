...
def test_contract_mismatch_is_rejected():
    mismatched = Observation(
        status="deviated", contract_id="other", runtime_digest="sha256:runtime",
        expected={}, actual={}, deviations=("mismatch",),
    )
    try:
        propose_with_lineage(attestation(), mismatched)
    except ValueError as exc:
        assert "contract IDs" in str(exc)
    else:
